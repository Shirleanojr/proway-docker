...
USAGE:  ./deploy.sh --REPO_DIR [REPO_DIR] --BRANCH [BRANCH] --LOG_FILE [LOG_FILE]
...
if [[ "$remote_sha" != "$local_sha" ]]; then
  echo "$(date '+%F %T') [DEBUG] new change detected $BRANCH @ $remote_sha" >> "$LOG_FILE"
  echo "$(date '+%F %T') [DEBUG] Stop App" >> "$LOG_FILE"
  docker compose -f "$WORK_DIR"/docker-compose.yml down
  
  echo "$(date '+%F %T') [DEBUG] Pull branch $BRANCH" >> "$LOG_FILE" 
  git pull origin $BRANCH

  echo "$(date '+%F %T') [DEBUG] Start up" >> "$LOG_FILE" 
  docker compose -f "$WORK_DIR"/docker-compose.yml up --build -d
  exit 0
fi
EOF
